...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        means0 = x_enc[:, :, 0, :].mean(1, keepdim=True).detach()
        x_enc = x_enc - means
        stdev = torch.sqrt(
            torch.var(x_enc, dim=1, keepdim=True, unbiased=False) + 1e-5)
        stdev0 = torch.sqrt(
            torch.var(x_enc[:, :, 0, :], dim=1, keepdim=True, unbiased=False) + 1e-5)
        x_enc /= stdev
        ...
        dec_out = dec_out * \
                  (stdev0[:, 0, :].unsqueeze(1).repeat(1, self.pred_len, 1))
        dec_out = dec_out + \
                  (means0[:, 0, :].unsqueeze(1).repeat(1, self.pred_len, 1))
        return dec_out
